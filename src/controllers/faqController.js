...
exports.getFaqById = async (req, res) => {
  try {
    const [faqs] = await db.query("SELECT * FROM faqs WHERE id = ?", [
      req.params.id,
    ]);
    if (faqs.length === 0) {
...
exports.createFaq = async (req, res) => {
  ...
  try {
    const insertPromises = faqs.map((faq) => {
      ...
      return db.query("INSERT INTO faqs (question, answer) VALUES (?, ?)", [
        question,
        answer,
      ]);
    });
    ...
    res
      .status(500)
      .json({ message: "Error creating FAQs", error: error.message });
  }
};
